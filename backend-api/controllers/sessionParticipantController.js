...
const setParticipantByDocumentId = async (req, res) => {
    ...
    try {
        ...
        const sessionParticipant = await SessionParticipant.create({
            session_id : collaborationSession.id,
            user_id : userId
        })
        ...
        console.error("Erreur lors de l'ajout d'un participant dans une session", error);
        res.status(500).json({ message: "Erreur serveur interne." });
    }
};
...
const deleteParticipantByDocumentId = async (req, res) => {
    ...
    try {
        ...
        if(!sessionParticipant) {
            console.error("Le participant recherché ne fait pas partie de la session du document d'id:", id)
            return res.status(500).json({ message : "Erreur serveur interne." })
        }
        ...
        console.error("Erreur lors de la suppression d'un participant dans une session", error);
        res.status(500).json({ message: "Erreur serveur interne." });
    }
};
